...
- **终端面板以「输入框覆盖层」占满会话视图区**：视图根节点带宿主的 `data-conversation-composer-overlay`（轨迹视图用的同一机制），输入框因此浮在面板之上、终端吃到完整高度，底部再按宿主发布的 `--dsh-composer-height` 留出内边距，最后一行不会被输入框盖住；输入框长高（多行输入、附件、任务条）改的是面板内容盒，由面板上的 `ResizeObserver` 收敛成一次重排。代价是这个模式下宿主会话列的宽度拖拽把手在终端 tab 激活期间不显示（宿主的既定行为），需要调整列宽时切回轨迹/对话即可；
...
8. 终端 tab 激活期间输入框浮在终端下方，**终端最后一行不被输入框盖住**；在输入框里多行输入（Shift+Enter）、加附件或出现任务条让输入框长高时，终端跟着收窄重排而不是被压住；
9. 终端 tab 激活期间宿主会话列的左右宽度拖拽把手不显示（覆盖层模式的既定行为）；切回「轨迹」/「对话」后把手恢复。
